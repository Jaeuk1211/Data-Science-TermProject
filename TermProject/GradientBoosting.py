...
grid_cv = GridSearchCV(gb_model, param_grid=params, cv=5)
grid_cv.fit(X_train,y_train)

print('Best hyper parameter: {0}'.format(grid_cv.best_params_))

gb_tuned = GradientBoostingClassifier(max_depth = 3,
                                  max_features = 3,
                                  learning_rate= 0.1,
                                  n_estimators = 100)

gb_tuned.fit(X_train_scale, y_train)
y_pred = gb_tuned.predict(X_test_scale)
print("RandomForestClassifier test accurarcy {}".format(gb_tuned.score(X_test_scale,y_test)))